variables {
  ...
  request_count_alarm_threshold = 1000
...
# Sentinel: require-terraform-tag — ManagedBy = "terraform" must be present
run "validate_managed_by_tag" {
  command = plan

  assert {
    condition     = aws_lb.example.tags["ManagedBy"] == "terraform"
    error_message = "ALB must carry ManagedBy = 'terraform' (required by Sentinel policy)"
  }

  assert {
    condition     = aws_security_group.web_sg.tags["ManagedBy"] == "terraform"
    error_message = "Web SG must carry ManagedBy = 'terraform' (required by Sentinel policy)"
  }

  assert {
    condition     = aws_cloudwatch_log_group.app.tags["ManagedBy"] == "terraform"
    error_message = "Log group must carry ManagedBy = 'terraform' (required by Sentinel policy)"
  }
}
...
run "validate_request_count_alarm" {
  command = plan

  assert {
    condition     = aws_cloudwatch_metric_alarm.high_request_count.threshold == var.request_count_alarm_threshold
    error_message = "Request count alarm threshold must match var.request_count_alarm_threshold"
  }

  assert {
    condition     = aws_cloudwatch_metric_alarm.high_request_count.metric_name == "RequestCount"
    error_message = "Request count alarm must use the RequestCount metric"
  }

  assert {
    condition     = aws_cloudwatch_metric_alarm.high_request_count.namespace == "AWS/ApplicationELB"
    error_message = "Request count alarm namespace must be AWS/ApplicationELB"
  }
}

# Sentinel: allowed-instance-types — only t2/t3 family allowed
run "validate_allowed_instance_type" {
  command = plan

  assert {
    condition     = can(regex("^t[23]\\.", aws_launch_template.example.instance_type))
    error_message = "Instance type must be t2 or t3 family (enforced by Sentinel allowed-instance-types policy)"
  }
}

run "validate_app_version_v5" {
  command = plan

  assert {
    condition     = var.app_version == "v5"
    error_message = "Day 22 module default app_version must be v5"
  }
}

...
variable "instance_type" {
  description = "EC2 instance type. Must be t2 or t3 family (enforced by Sentinel policy)."
  type        = string
  default     = "t3.micro"
...
variable "ami" {
  description = "AMI ID override. Leave empty to use the latest Amazon Linux 2023."
  type        = string
  default     = ""
}
...
variable "app_version" {
  description = "Application version string rendered into the HTML response. Bump to trigger zero-downtime replacement."
  type        = string
  default     = "v5"
}

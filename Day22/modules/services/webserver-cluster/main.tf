locals {
  # ManagedBy = "terraform" is required by the Sentinel require-terraform-tag policy.
  # Every resource in this module inherits this tag via merge(local.common_tags, ...).
  common_tags = merge(
    {
...
resource "aws_cloudwatch_metric_alarm" "high_request_count" {
  ...
  alarm_description   = "ALB request count > ${var.request_count_alarm_threshold}/min on ${var.cluster_name}"
  treat_missing_data  = "notBreaching"

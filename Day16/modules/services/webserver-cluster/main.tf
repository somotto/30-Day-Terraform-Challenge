locals {
  ...
  effective_instance_type = var.instance_type
}
...
# Allow instances to publish custom metrics to CloudWatch
...
resource "aws_security_group" "web_sg" {
  ...
  description = "Instances: allow traffic only from ALB, no direct internet access"
  vpc_id      = data.aws_vpc.default.id
...
resource "aws_security_group_rule" "web_in_from_alb" {
  ...
  description              = "Allow traffic only from ALB - no direct internet access"
}
...
# High CPU alarm — fires when average CPU > threshold for 2 consecutive 2-minute periods
resource "aws_cloudwatch_metric_alarm" "high_cpu" {
  ...
  alarm_description   = "Triggers when average CPU exceeds ${var.cpu_alarm_threshold}% for 4 minutes on ${var.cluster_name}"
...
# Unhealthy host alarm — fires when any target is unhealthy
resource "aws_cloudwatch_metric_alarm" "unhealthy_hosts" {
  ...
  statistic           = "Maximum"
  threshold           = 0
  alarm_description   = "Triggers when any target in ${var.cluster_name} target group is unhealthy"
  treat_missing_data  = "notBreaching"
...
resource "aws_cloudwatch_metric_alarm" "alb_5xx" {
  ...
  statistic           = "Sum"
  threshold           = 10
  alarm_description   = "Triggers when ALB returns more than 10 5xx errors per minute for ${var.cluster_name}"
  treat_missing_data  = "notBreaching"

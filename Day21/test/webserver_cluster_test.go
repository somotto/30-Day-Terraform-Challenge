...
// TestWebserverClusterIntegration deploys the module, verifies all outputs
func TestWebserverClusterIntegration(t *testing.T) {
	...
	opts := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		...
		Vars: map[string]interface{}{
			...
			"min_size":                       1,
			"max_size":                       2,
			"app_version":                    "v4-integration-test",
	...
	// Verify the request count alarm ARN contains the cluster name
	...
	assert.Contains(t, body, "v4", "response must include app_version v4")
}
